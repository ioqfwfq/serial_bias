function [raw_startpoint, raw_endpoint, startpoint, endpoint, newy, newx] = Find_Saccade(AllData,ntr)
% This function finds indices of the startpoint and endpoint of a saccade for a single trial
...
indexON = find(AllData.trials(ntr).eye_time(:)>=(AllData.trials(ntr).timestamp_queue(5)-AllData.trials(ntr).time));
